...
outhead = inhead + '\tvarL\tflank\tflankScore\tmhL\tmh1L\thom\tnbMM'
outhead += '\tmhDist\tMHseq1\tMHseq2\tpamMot\tbestPamHet\tpamUniq\tguidesNoOT\tguidesMinOT'
variant_output_file.write(outhead + '\n')
gouthead = outhead + '\tprotospacer\tmm0\tmm1\tmm2\tm1Dist1\tm1Dist2\tmhDist1\tmhDist2'
gouthead += '\tnbOffTgt\tlargestOffTgt\tbotScore\tbotSize\tbotVarL\tbotGC\tbotSeq\n'
...
for input_line in variant_input_file:
    ...
    for flank in [1, 2]:
        if(flank == 1):  # Using the alignment score, the best flank is chosen
            mhfl = mhfl1
        ...
        # too short first microhomology stretch, jump to the next input line
        if(mhfl['score'] == 0 or mhfl['mhL'] < args.minMHL or
           mhfl['hom'] < args.minhom or mhfl['m1L'] < args.minm1L):
            continue
        # Find PAM motives
        pams = findPAM(varseq, fl1seq, fl2seq, mhfl, args.maxTail,
                       args.pamseq, pamseq_rev, args.pamcut)
        ...
        voutline = input_line_raw + '\t' + str(vsize) + '\t' + str(flank)
        voutline += '\t' + str(mhfl['score']) + '\t' + str(mhfl['mhL']) + '\t'
