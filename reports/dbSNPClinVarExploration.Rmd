...
```{r}
mean(cv$GENEINFO == cv$GENEINFO.ClinVar)
subset(cv, GENEINFO != GENEINFO.ClinVar) %>% select(GENEINFO, GENEINFO.ClinVar) %>% head %>% kable
```
...
dup.dt = dup.dt[, .N, by=nbentry]
ggplot(dup.dt, aes(x=factor(nbentry), y=N)) + geom_bar(stat='identity') + theme_bw() + xlab('number of entry with same coordinates') + ylab('variants')
```
