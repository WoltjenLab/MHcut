...
var = fread('gunzip -c ../data/mhcut-dbsnp-clinvar-deletion-variants.tsv.gz', select=c(20:22,25,29:30))
```
...
```{r}
guides = fread('gunzip -c ../data/mhcut-dbsnp-clinvar-deletion-guides.tsv.gz', sel=c(1:4,21,29,30,43))
nb.mh3.pam.uniq.nonmh = nrow(unique(guides[mhL>=3 & pamMot>0 & pamUniq>0 & nbNMH==0, .(chr, start, end, RS)]))
```
...
```{r}
var.x = fread('gunzip -c ../data/mhcut-dbsnp-clinvar-deletion-xCas9-variants.tsv.gz', select=c(20:22,25,29:30))
guides.x = fread('gunzip -c ../data/mhcut-dbsnp-clinvar-deletion-xCas9-guides.tsv.gz', sel=c(1:4,21,29,30,43))
...
xcas9.sum = tibble(cas9='xCas9',
                   ...
                   nb.mh3.pam.uniq.nonmh = nrow(unique(guides.x[mhL>=3 & pamMot>0 & pamUniq>0 & nbNMH==0, .(chr, start, end, RS)])))
